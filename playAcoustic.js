function playAcoustic() {
  let pressColor = '#768d8e'; //color when key is pressed
  ...
  // Generate keyboard
  let visualKeyboard = document.getElementById('acoustic');
  let selectSound = {
    value: '2' //acoustic guitar
  };
  ...
  for (var i = -2; i <= 1; i++) {
    for (var n in notes) {
      if (n[2] != 'b') {
        var thisBtn = document.createElement('div');
        if (n.length > 1) {
          //adding sharp sign makes 2 characters
          thisBtn.className = 'magenta btn'; //2 classes
          thisBtn.style.width = '40px';
          thisBtn.style.height = '40px';
          thisBtn.style.left = 40 * iWhite + 'px';
        } else {
          thisBtn.className = 'indigo btn';
          thisBtn.style.width = '60px';
        ...
        thisBtn.appendChild(label);
        thisBtn.setAttribute('ID', 'KEY_' + n + ',' + i);
        const keyCode = reverseLookup[n + ',' + i];
        thisBtn.addEventListener('mousedown', () => {
          fnPlayKeyboard({ keyCode });
        });
        visualKeyboard[n + ',' + i] = thisBtn;
        visualKeyboard.appendChild(thisBtn);
